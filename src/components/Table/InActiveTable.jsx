...
// const InactiveAdminTable = () => {
//   return (
//     <div>InactiveAdminTable</div>
//   )
// }

// export default InactiveAdminTable
...
const InactiveAdminTable = () => {
  ...
  const getAllListingData = async () => {
    try {
      ...
      const activeData = res.data.filter((item) => item.status === "inactive");
      ...
      <div className="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8">
        <div className="align-middle rounded-tl-lg rounded-tr-lg inline-block w-full overflow-hidden px-2 py-3">
          <div className="flex justify-between items-center">
            <div>
              InActive Listing{" "}
              <span className="text-red-400 p-1">{allListing?.length}</span>
            </div>
            <div>
              <Link
                to="/active-listing"
              ...
              <Link
                to="/pending-listing"
                className="inline-flex items-center mx-2 justify-center px-3 py-1 font-sans font-semibold tracking-wide text-white text-xs bg-orange-400 hover:bg-orange-500  rounded-sm shadow-2xl"
              >
                Pending
              </Link>
              <Link
